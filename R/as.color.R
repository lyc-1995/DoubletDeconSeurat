...
as.color <- function(
  ...
  if(numcols > 255^3) {
    warning('The input vector has more unique values than possible colors, some colors will be recycled.')
  }
  #create colors
  r <- sample(colseq, length(x = numcols), replace = FALSE)
  g <- sample(colseq, length(x = numcols), replace = FALSE)
  b <- sample(colseq, length(x = numcols), replace = FALSE)
  colz <- rgb(red = r, green = g, blue = b, alpha = alpha)

  #assign colors to input values
  colvec <- rep(NA, length(x = x))
  for (i in 1:length(x = numcols)) {
